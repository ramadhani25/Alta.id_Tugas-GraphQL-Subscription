...
const App = () => {
  ...
  // Subscription
  const { data: dataAllTodoSub, loading: loadingAllTodoSub } =
    useSubscription(getAllTodosSub);

  // Mutation
  const [insertTodo, { loading: loadingAddTodo }] = useMutation(addTodo, {
    refetchQueries: [getTodosByUserId],
  });
  ...
  const onClickItem = (idx) => {
    const item = dataAllTodoSub?.todos.find((todo) => todo.id === idx);
    updateTodoComplete({
      variables: {
        id: idx,
  ...
  if (loadingAddTodo || loadingAllTodoSub || loadingTodoById) {
    return (
